...
def star(x: float, y: float, size: float, color: str) -> None:
    '''
    Draws a five-point star with its top-left arm at
    position (x, y). The distance between the top-left arm
    and top-right arm is 'size'. The color of the lines drawn
    is 'color'.
    '''
    turtle.teleport(x, y)
    turtle.color(color)
